...
from old.flight_tracker import FlightTracker
from old.csv_logger import CSVLogger
...
def main():
    ...
    def process_beacon(raw_message):
        ...
        # Check for dropped signals every ~30 seconds
        now = time.time()
        if now - last_timeout_check > 30:
            current_timestamp = beacon.get("timestamp") or datetime.now(timezone.utc)
            timeout_events = tracker.check_timeouts(current_timestamp)
            for te in timeout_events:
                logger.log(te)
                print(format_event(te) + " (SIGNAL DROP)")
